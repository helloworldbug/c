...
function fmacapi_set_data(str, pwd) {
...
function fmacapi_get_data(str, pwd) {
...
//  ----------------------------------
//
//   Project      : H5微场景移动应用V1.0
//   Module	    : Web App前端公共方法
//   By           : YC.Q
//   When		  : 2014.8.27
//   Updated by   :
//   Updated when :
...
/**
 *注册命名空间，参数为命名空间全路径，如"fmacloud"
 */
function fmacapi_RegisterNameSpace(fullName) {
...
/**
 * 生成唯一对象id
 * @param prefix - id前缀，可以使用对象类型，例如'tpl'，'page'等；
 * @returns {string}
 */
function fmacapi_create_uid(prefix){
...
function fmacapi_filename_fromurl(url){

    var pos = url.lastIndexOf('/');
    return url.substr(pos+1,url.length-pos-1 );
}

function fmacapi_report_exception(e){
    alert("一个错误发生在"+ e.fileName+"。\r\n错误信息："+ e.message);
}

function fmacapi_is_native(){

    if ( ! window.fma_apptype ){
        window.fma_apptype = fmacapi_GetRequest()['app'] || "web";
    }
...
//2015-01-21
//1：增加微信分享url类型时的缩略图支持

//2015-01-23
//1: 增加图片多选接口
//2：增加读取图片接口
//3: 增加QQ分享url类型时的缩略图支持
...
//2015-02-02
//1: 增加 fmantv_get_build_info 返回pack name

//2015-07-02
//1: 增加 fmantv_download_multi_files_bg
//2: 增加 fmantv_download_cancel
//3: 增加 fmantv_read_file_from_local_path
//4: 增加 fmantv_delete_local_file

//2015-07-03
//1: 增加 fmantv_appstore_score
...
function fmantv_init (jspath,cb_ok,cb_err) {
	...
	if ( os_type==cst.OS_WIN ) {
		...
		if ( os_type=="mac" )
			...
			load_js(jspath+"cordova-android.js");
	}
...
*@descript  分享的描述
*/
function fmantv_share (user_id, share_obj, obj_type, dst_type, url_thumb, cb_ok,cb_err,title,descript) {
   ...
   exec( function () {
			...
			if (cb_err) cb_err(err);
		}, "gliplug", "fmantv_share", [user_id,share_obj,obj_type, dst_type, url_thumb,title,descript]);
   return 0;
}
...
/**
 *后台下载文件到本地,返回本地文件路径
 *@cb_ok(local_paths) 调用成功后，异步回调接口返回
 *@cb_err(err) 调用失败后，异步回调接口返回
 *@cb_prog(value) 下载进度回调函数, value 0 ~ 100
 *@返回本次操作的一个句柄， 可用来作为取消下载的参数
 */
function fmantv_download_multi_files_bg (urls, cb_ok, cb_err, cb_prog) {
    if ( fmantv_is_inited==false ) return cst.E_NOT_INIT;
    var exec = cordova.require("cordova/exec");
    
    var t_hand = magic++;
    
    list_prog[t_hand] = cb_prog;
    exec( function (arr) {
            if (cb_ok) cb_ok(arr);
         }, function (err) {
            if (cb_err) cb_err(err);
         }, "gliplug", "fmantv_download_multi_files_bg",
                [urls, t_hand]);
    return t_hand;
}
...
/**
 *取消下载
 *@hand 要取消的操作句柄, 由fmantv_download_multi_files_bg返回
 */
function fmantv_download_cancel (hand) {
    if ( fmantv_is_inited==false ) return cst.E_NOT_INIT;
    var exec = cordova.require("cordova/exec");
    exec( function () {
         }, function () {
         }, "gliplug", "fmantv_download_cancel",
         [hand]);
    return 0;
}

/**
 *从本地文件路径读取一个文件，返回base64
 *@local_path 文件保存的本地路径
 *@cb_ok(base64) 调用成功后，异步回调接口返回
 *@cb_err(err) 调用失败后，异步回调接口返回
 */
function fmantv_read_file_from_local_path (local_path, cb_ok, cb_err) {
    if ( fmantv_is_inited==false ) return cst.E_NOT_INIT;
    var exec = cordova.require("cordova/exec");
    exec( function (base64) {
         if (cb_ok) cb_ok(base64);
         }, function (err) {
         if (cb_err) cb_err(err);
         }, "gliplug", "fmantv_read_file_from_local_path",
         [local_path]);
    return 0;
}
...
/**
 *删除一个本地文件
 *@local_path 要删除的文件本地路径
 *@cb_ok(local_path) 调用成功后，异步回调接口返回
 *@cb_err(err) 调用失败后，异步回调接口返回
 */
function fmantv_delete_local_file (local_path, cb_ok, cb_err) {
    if ( fmantv_is_inited==false ) return cst.E_NOT_INIT;
    var exec = cordova.require("cordova/exec");
    exec( function (local_path) {
         if (cb_ok) cb_ok(local_path);
         }, function (err) {
         if (cb_err) cb_err(err);
         }, "gliplug", "fmantv_delete_local_file",
         [local_path]);
    return 0;
}
...
/**
 * 返回包信息
 * cb_ok (channel, para1, para2, pack_name,rsd1,rsd2,rsd3,rsd4,rsd5,rsd6)
 * channel - 渠道号， 内部测试版本返回空
 * pack_name - 包名
 */
function fmantv_get_build_info (cb_ok, cb_err) {
    if ( fmantv_is_inited==false ) return cst.E_NOT_INIT;
    var exec = cordova.require("cordova/exec");
    exec( function (arr) {
         //alert(arr[0] + " " + arr[1] + " " + arr[2] );
         if (cb_ok)
            cb_ok( arr[0], arr[1], arr[2],arr[3],arr[4],arr[5],arr[6],arr[7],arr[8],arr[9]);
         }, function (err) {
         if (cb_err)
            cb_err(err);
 ...
 * 检测第3方app是否安装
 * app - 类型， wx:微信
 * cb_ok (true) 安装，cb_err (false) 没有安装或者api不支持
 */
function fmantv_is_3th_app_installed (app, cb_ok, cb_err) {
    if ( fmantv_is_inited==false ) return cst.E_NOT_INIT;
    var exec = cordova.require("cordova/exec");
    exec( function () {
         //alert(arr[0] + " " + arr[1] + " " + arr[2] );
         ...
         if (cb_err)
            cb_err(false);
         }, "gliplug", "fmantv_is_3th_app_installed", [app]);
    return 0;
}
 ...
 * 得到登陆授权
 * app - 类型， wx:微信
 * cb_ok (code) 授权成功，code-返回的授权码，cb_err () 授权失败
 */
function fmantv_3th_app_auth(app, cb_ok, cb_err) {
    if ( fmantv_is_inited==false ) return cst.E_NOT_INIT;
    var exec = cordova.require("cordova/exec");
    exec( function (code) {
         //alert(arr[0] + " " + arr[1] + " " + arr[2] );
         ...
         if (cb_err)
            cb_err(err);
         }, "gliplug", "fmantv_3th_app_auth", [app]);
    return 0;
}

/**
 * 用户选择图片
 * cb_ok (photos) 成功 photos-用户选择的图片路径数组，cb_err () 失败
 */
function fmantv_select_photos (cb_ok, cb_err) {
    if ( fmantv_is_inited==false ) return cst.E_NOT_INIT;
    var exec = cordova.require("cordova/exec");
    exec( function (photos) {
         //alert(arr[0] + " " + arr[1] + " " + arr[2] );
         if (cb_ok)
            cb_ok(photos);
         }, function (err) {
...
/**
 * 读取图片数据，返回base64编码
 * asset_url fmantv_select_photos返回的图片路径
 * quality - 精度 ( 0 ~ 100 )
 * cb_ok (base64_data) 成功，cb_err () 失败
 */
function fmantv_read_photo (asset_url,quality,cb_ok, cb_err) {
    if ( fmantv_is_inited==false ) return cst.E_NOT_INIT;
    var exec = cordova.require("cordova/exec");
    exec( function (base64_data) {
         //alert(arr[0] + " " + arr[1] + " " + arr[2] );
         if (cb_ok)
            cb_ok(base64_data);
         }, function (err) {
         if (cb_err)
            cb_err(err);
         }, "gliplug", "fmantv_read_photo", [asset_url,quality]);
    return 0;
}
...
*@descript  分享的描述
*/
fmantv.share = function (user_id, share_obj, obj_type, dst_type, url_thumb,cb_ok,cb_err,title,descript) {
	fmantv_share (user_id, share_obj, obj_type, dst_type, url_thumb,cb_ok,cb_err,title,descript);
}
 ...
 *@返回本次操作的一个句柄， 可用来作为取消下载的参数
 */
fmantv.download_multi_files_bg = function (urls, cb_ok, cb_err, cb_prog) {
    return fmantv_download_multi_files_bg (urls, cb_ok, cb_err, cb_prog);
}
 ...
 *@hand 要取消的操作句柄, 由fmantv_download_multi_files_bg返回
 */
fmantv.download_cancel = function (hand) {
    fmantv_download_cancel (hand);
}
 ...
 *@cb_err(err) 调用失败后，异步回调接口返回
 */
fmantv.read_file_from_local_path = function (local_path, cb_ok, cb_err) {
    fmantv_read_file_from_local_path (local_path, cb_ok, cb_err);
}
 ...
 *@cb_err(err) 调用失败后，异步回调接口返回
 */
fmantv.delete_local_file = function (local_path, cb_ok, cb_err) {
    fmantv_delete_local_file (local_path, cb_ok, cb_err);
}
 ...
 * pack_name - 包名
 */
fmantv.get_build_info = function(cb_ok, cb_err) {
    fmantv_get_build_info(cb_ok, cb_err);
}
 ...
 * cb_ok (true) 安装，cb_err (false) 没有安装或者api不支持
 */
fmantv.is_3th_app_installed = function(app, cb_ok, cb_err) {
    fmantv_is_3th_app_installed (app, cb_ok, cb_err);
}
 ...
 * cb_ok (code) 授权成功，code-返回的授权码，cb_err () 授权失败
 */
fmantv._3th_app_auth = function(app, cb_ok, cb_err) {
    fmantv_3th_app_auth (app, cb_ok, cb_err);
}
 ...
 * cb_ok (photos) 成功 photos-用户选择的图片路径数组，cb_err () 失败
 */
fmantv.select_photos = function(cb_ok, cb_err) {
    fmantv_select_photos (cb_ok, cb_err);
}

/**
 * 读取图片数据，返回base64编码
 * asset_url fmantv.select_photos返回的图片路径
 * quality - 精度 ( 0 ~ 100 )
 * cb_ok (base64_data) 成功，cb_err () 失败
 */
fmantv.read_photo = function(asset_url,quality,cb_ok, cb_err) {
    fmantv_read_photo(asset_url, quality,cb_ok, cb_err);
}
...
//
//   Project      : H5微场景移动应用V1.0
//   Module	    : Web App云端功能接口
//   By           : YC.Q
//   When		  : 2014.8.27
//   Updated by   :
//   Updated when : 2014.10.17
//   Title        :
//   Copyright    : H5微场景
//
//   reviewed:
...
function _fmacloud_execute_queue() {
    ...
    var proto = fmacloud.Object.prototype;
    var queue = proto.task_queue;
    if (queue.length > 0) {

        var params = queue.shift();

        var objarray = params.get("arg1");
        var index = params.get("arg2");
        var cb_task = params.get("arg3");
        var cb_ok = params.get("arg4");
        var cb_err = params.get("arg5");
        var interval = params.get("arg6");

        cb_task(objarray[index], function (obj) {

                if (index < objarray.length - 1) {

                    params.set("arg2", index + 1);
                    proto.task_queue.push(params);

                    if (interval == null) interval = 0;

                    setTimeout("_fmacloud_execute_queue()", interval);
...
//注册名字空间
//    fmacapi_RegisterNameSpace("fmacloud"); 

    /*
     //fmacoud模块初始化方法
     */
    window.fmacloud = AV;
    fmacloud.init_AV = AV.init;
    fmacloud.initialize = function (channel, para1, para2) {
        fmacloud.init_AV({
            appId: para1,
            appKey: para2
        });

        //fmacloud.init_AV(para1, para2);//旧的方式
     ...
     *模版或作品查询
     *模版/作品/签章查询
     *objsel:选择查询模版、作品还是签章，可以是以下值(int型)或以下值的数组(Array)：
     *fmaobj.TPL_SINGLE_TPL =  0;   //单页模版
     *fmaobj.TPL_SINGLE_WORKS = 1;  //单页作品
     *fmaobj.TPL_SINGLE_TASTER = 2; //签章
     *fmaobj.TPL_SINGLE_CHECKED = 3;//已通过审核的作品
     *fmaobj.TPL_MULTI_TPL = 10;      //多页模板
     *fmaobj.TPL_MULTL_WORKS = 11;    //多页作品
     *fmaobj.TPL_FORMAT   = 12;       //版式
     *fmaobj.TPL_MULTI_CHECKED = 13;  //已通过审核的多页作品
     *fmaobj.TPL_CNT_APP  = 20;       //内容应用
     *funcsel:查询选择器，其取值为：
     ...
     *orderby:设定排序字段
     *isAscending: 布尔值，是否按升序排列
     *skip: 在符合条件的对象中跳过多少个，用于分页查询
     *limit: 限制获取的对象数
     *@cb_ok(tpl_objs/works_objs) 调用成功后，异步回调接口返回tpl_objs模版对象数组(fmaobj.tpl对象数组)
     *      或works_objs作品对象数组(fmaobj.works对象数组）
     *@cb_err(err) 调用失败后，异步回调接口返回,err包含错误信息（字符串）
     */
    fmacloud.query_tpl_works = function (objsel, funcsel, orderby, isAscending, skip, limit, cb_ok, cb_err) {

        //查询云端模版/作品
        var tpl_works = fmacloud.Object.extend("tplobj");
        var query = new fmacloud.Query(tpl_works);

        if ( Object.prototype.toString.call(objsel) === '[object Array]') {
            query.containedIn("tpl_type",objsel);
        } else {

            query.equalTo("tpl_type", objsel);
        }
        query.skip(skip);
        query.limit(limit);
        ...
        switch (funcsel) {
            ...
            case 'onsale':
                ...
                break;
            case 'not onsale':
                query.equalTo("approved", 0);
                break;
            case 'editor rec':
                query.greaterThan("approved", 0);
                query.greaterThan("editor_recno", 0);
                query.ascending("editor_recno");

                break;
            case 'author rec':
                query.greaterThan("approved", 0);
                query.greaterThan("author_recno", 0);
                query.ascending("author_recno");

                break;
            default:

                var label = funcsel.split(':');
                if (label.length > 1) {
        ...
        if (!failed) {
            ...
            cb_err(fmacloud.Error(-1, fmacloud.Error.REQUEST_NOT_SUPPORTED));
        }
    ...
    /**
     *模版/作品/签章查询
     *tpl_works_id: 待获取的tplobj的id：
     *@cb_ok(tplobj) 调用成功后，异步回调接口返回tpl_obj对象
     *@cb_err(err) 调用失败后，异步回调接口返回,err包含错误信息（字符串）
     */
    fmacloud.get_tpl_works_obj = function (tpl_works_id,cb_ok,cb_err,incReadCount){

        var tpl_obj = fmacloud.Object.extend("tplobj");
        var query = new fmacloud.Query(tpl_obj);

        query.equalTo("tpl_id", tpl_works_id);
        query.find().then(function (dobjs) {
            if ( dobjs.length > 0 ){
                var tpl = dobjs[0];

                if ( !! incReadCount ){
                    tpl.set("read_int", tpl.get("read_int")+1 );
                    tpl.save();
                }
                cb_ok(tpl);

            }else{
                cb_err("tpl object not found:"+tpl_works_id);
            }
        },function(err){
            cb_err(err.message);
        });
     ...
     *贴纸/边框查询
     *objsel:对象名
     *funcsel:功能查询选择器，其取值为：
     *          'label:xxx' -根据标签选择(多个标签用冒号隔开）
     *          'type:1/2' -选择类型(1为贴纸，2为边框)
     *          'type_and_scale:2:escape("1:1")' -选择类型和比例(1为贴纸，2为边框)
     *          'all' -查询所有数据，默认按创建日期降序排列
     *orderby:设定排序字段
     *isAscending: 布尔值，是否按升序排列
     *skip: 在符合条件的对象中跳过多少个，用于分页查询
     *limit: 限制获取的对象数
     *@cb_ok(corpus) 调用成功后，异步回调接口返回corpus对象数组(
     *@cb_err(err) 调用失败后，异步回调接口返回,err包含错误信息（字符串）
     */
    fmacloud.query_res = function (objsel, funcsel, orderby, isAscending, skip, limit, cb_ok, cb_err) {

        //查询语料
        var data_obj = fmacloud.Object.extend(objsel);
        var query = new fmacloud.Query(data_obj);
                ...
                if (label.length > 1) {
                    ...
                    switch (label[0]) {
                        ...
                        case "type":
                            ...
                            var type_no = parseInt(label[1]);

                            query.equalTo("category", type_no);
                            break;
                        ...
                        case "type_and_scale":
                            ...
                            var scale = unescape(label[2]);

                            query.equalTo("category", type_no);
                            query.equalTo("scale", scale);
     ...
     *@cb_err(err) 调用失败后，异步回调接口返回,err包含错误信息（字符串）
     */
    fmacloud.query_corpus = function (objsel, funcsel, orderby, isAscending, skip, limit, cb_ok, cb_err) {
    ...
    /**
     *标签查询
     *key:关键字（即对象属性名），如为null，则查询所有标签
     *value:要求key进行匹配的值
     *skip: 在符合条件的对象中跳过多少个，用于分页查询
     *limit: 限制获取的对象数
     *@cb_ok(labelobj) 调用成功后，异步回调接口返回labelobj对象数组
     *@cb_err(err) 调用失败后，异步回调接口返回,err包含错误信息（字符串）
     */
    fmacloud.query_labels = function (key, value, orderby, isAscending, skip, limit, cb_ok, cb_err) {

        //查询标签
        var data_obj = fmacloud.Object.extend("labels");
        var query = new fmacloud.Query(data_obj);
    ...
    /*
     //针对一个数组，执行异步任务，其中：
     //objarray  - 待执行任务的数组
     //index     - 从第几个元素开始处理，一般从0开始
     //cb_task(obj,cb_ok,cb_err)  - 处理单个数组元素的方法
     //cb_ok(objarray)   - 处理成功的回调函数
     //cb_err(error)     -处理失败的回调函数
     */
    fmacloud.execute_array_task = function (objarray, index, cb_task, cb_ok, cb_err) {

        //var glb = GetGlobalObj();
        //glb._eat_objarray=objarray;
        //glb._eat_index=index;
        cb_task(objarray[index],
            function (obj) {

                //var glb = GetGlobalObj();
                //var objarray = glb._eat_objarray;
                //var index = glb._eat_index;
                if (index < objarray.length - 1) {
                    fmacloud.execute_array_task(objarray, index + 1, cb_task, cb_ok, cb_err);
                }
                else {
                    cb_ok(objarray);
                }
    ...
    /*
     //针对一个数组，执行异步任务，支持两个任务之间的异步延时。其中：
     //objarray  - 待执行任务的数组
     //index     - 从第几个元素开始处理，一般从0开始
     //cb_task(obj,cb_ok,cb_err)  - 处理单个数组元素的方法
     //cb_ok(objarray)   - 处理成功的回调函数
     //cb_err(error)     -处理失败的回调函数
     //interval          -每个任务执行前延时的毫秒数
     */
    fmacloud.execute_array_task_async = function (objarray, index, cb_task, cb_ok, cb_err, interval) {

        fmacloud.init_queue_task();

        var proto = fmacloud.Object.prototype;
        var params = fmaobj.params.create();
        params.set("arg1", objarray);
        params.set("arg2", index);
        params.set('arg3', cb_task);
        params.set("arg4", cb_ok);
        params.set("arg5", cb_err);
        params.set("arg6", interval);
        proto.task_queue.push(params);
    ...
    /*
     // 递归获取数组对象的数据
     */
    fmacloud._fetch_object = function (objarray, index, cb_ok, cb_err) {

        fmacloud.execute_array_task(objarray, index,
    ...
    fmacloud.get_tpl_works_data = function (tw_id, cb_ok, cb_err) {

        var data_obj = fmacloud.Object.extend("pages_data");
        var query = new fmacloud.Query(data_obj);

        query.equalTo("key_id", tw_id);
        query.find().then(function (dobjs) {

            if (dobjs.length > 0) {
                var dobj = dobjs[0];
                var pages = dobj.get("pages");
                //GetGlobalObj()._gtwd_tpl_data = dobj;

                //获取每个页对象数据
                fmacloud._fetch_object(pages, 0, function (pages) {

                    //

                    //获取每个page的item数据(elem元素)
                    fmacloud.execute_array_task(pages, 0,
                        function (page, cb_ok, cb_err) {

                            var items = page.get("item_object");
                            fmacloud._fetch_object(items, 0, cb_ok, cb_err);
                        },
                        function (obj) {
                            ...
                            if (page.get("item_object")[0].get("item_id") != null) {
                                ...
                                cb_err(new fmacloud.Error(3201, "元素数据可能已受到损坏。"));
                            }
                        },
                        cb_err
                    );
                ...
                cb_err(new fmacloud.Error(3202, "指定ID的数据不存在。"));
            }
        }, function (error) {
            cb_err(error);
        });
    ...
    fmacloud.erase_tpl_works = function (tw_id, tw_obj, cb_ok, cb_err) {
        ...
        try {
            ...
            if ( tw_obj !== null  ) {
                tw_obj.destroy({
                    success: function (myObject) {
                        //alert("删除对象成功：" + myObject.get("objectid"));
                        cb_ok(myObject);
                    },
                    error: function (myObject, error) {
                        alert("删除对象失败：" + myObject.get("objectid") + "原因是：" + error);
                        cb_err(myObject, error);
                    }
                });
            }
            ...
            query.find({
                success: function (dobjs) {
                    ...
                    for (var i = 0; i < dobjs.length; i++) {
                                        ...
                                        elem.fetch({
                                            success: function (elem) {
                                                ...
                                                if (elem.get("item_cntype") == 2) {
                                                    filename = fmacapi_filename_fromurl(elem.get("item_val"));
                                                }
                                                else if (elem.get("item_cntype") == 3) {
                                                    filename = elem.get("item_val");
                                                }

                                                if (filename != null) {
                                                    filename = dobj.get("key_id") + "." + filename;

                                                    var file = fmacloud.Object.extend("_File");
                                                    var query = new fmacloud.Query(file);
                                                    query.equalTo("name", filename);
                                                    query.find({
                                                        success: function (files) {
                                                            if (files.length > 0) {

                                                                var file = new fmacloud.File();
                                                                file.id = files[0].id;
                                                        ...
                                                        error: function (error) {
                                                            alert("查找文件对象失败: " + error.code + " " + error.message);
                                                        }
                                                    });
                                                }
                        ...
                        if ( dobj !== null ) {
                            dobj.destroy({
                                success: function (myObject) {
                                    //alert("删除数据成功：" + myObject.get("objectid"));
                                },
                                error: function (myObject, error) {
                                    alert("删除数据失败：" + myObject.get("objectid") + "原因是：" + error);
                                }
                            });
                        }
                    }
                ...
                error: function (err) {
                    alert("查找数据失败：" + err.message);
                }
            });
        } catch (e) {
            //fmacapi_report_exception(e);
...
/////////////////////////////////////////////
    ...
    function fma_getMSec() {

        var myDate = new Date();
        return myDate.getMilliseconds();
    }

    fmacloud._File_withlogoUrl = function (url,dx, dy, logoUrl, opacity, gravity) {

        if (!url)throw"Invalid url.";
        if (!dx || !dy || 0 >= dx || 0 >= dy)throw"Invalid dx or dy value.";
        if (!logoUrl)throw"Invalid logoUrl.";

        opacity = opacity || 100;
        gravity = gravity || "SouthEast";
        return url + "?watermark/1/image/" + base64encode(logoUrl)
            + "/dissolve/" + opacity
            + "/gravity/" + gravity
            + "/dx/" + dx
            + "/dy/" + dy;
    ...
    fmacloud.tpl_withlogo_img_url = function (img_url, logo_url, dx, dy, cb_ok, cb_err, opacity, position) {

        var url = img_url;

        var files = fmacloud.Object.extend("_Files");
        var query = new fmacloud.Query(files);

        query.equalTo("url", url);
        query.find().then(function (dobjs) {

            if (dobjs.length > 0) {
                cb_ok(fmacloud._File_withlogoUrl(dobjs[0].url(),dx, dy, logo_url, opacity, position));
            } else {
                cb_err(fmacloud.Error(3203, fmacloud.Error.OBJECT_NOT_FOUND));
            }
        }, cb_err);
    };
    ...
    /*
     **  将img_base64字符串保存为图片
     *   参数：fileformat, 文件格式(.png或.jpg)
     *   cb_ok(file)，成功回调函数的参数为图片file对象
     *   cb_err(error)，失败回调函数参数为错误信息(字符串)
     */
    fmacloud.save_image = function (fileformat,img_base64_str, cb_ok, cb_err) {
        ...
        if (img_base64_str != null && img_base64_str != "") {

            var file = new fmacloud.File(fmacapi_create_uid()+fileformat, { base64: img_base64_str });
    ...
    fmacloud.tpl_save_effect_img = function (tplobj, img_base64_str, cb_ok, cb_err) {

        if (img_base64_str != null && img_base64_str != "") {

            //var filename = tplobj.get("effect_img");
            //if (filename.substr(0, 3) == "AV:") {
            //    filename = filename.slice(3);
            //}
            var filename = fmacapi_create_uid();

            var file = new fmacloud.File(filename + ".jpg", { base64: img_base64_str });
            file.save().then(function () {

                //效果图保存成功
                tplobj.set("effect_img", "AV:" + file.url());

                //获取缩略图
                //计算宽度和高度
                var tw = Math.round(640 / 2); //虚屏宽度的1/2
                var quality = 80;
                var tplw = tplobj.get("tpl_width");
                var tplh = tplobj.get("tpl_height");
                var th = Math.round( tplh * tw / tplw);

                var thumburl = file.thumbnailURL(tw, th, quality, true, "jpeg");
                tplobj.set("tiny_img", "AV:" + thumburl);

                //保存分享图片
                if ( tplw >= tplh ){
                    th = 200;
                    tw = Math.round( tplw * th / tplh );
                }else{
                    tw = 200;
                    th = Math.round( tplh * tw / tplw);
                }

                thumburl = file.thumbnailURL(tw, th, quality, true, "jpeg");
                tplobj.set("tpl_share_img", "AV:" + thumburl);
    ...
    fmacloud.saveMEBook = function (tplobj, tpldata, tpl_imgs, cb_ok, cb_err, cb_progress) {
        cb_progress = cb_progress || function () {
        };
        try {
            /*
             * 错误检查
            ...
            var errmsg, c_groups = [], group_object = [];;
            if (tplobj.get("tpl_width") <= 0 || tplobj.get("tpl_height") <= 0) {
                errmsg = "模版数据不正确（宽或高小于等于零）。";

                fmacapi.set_last_error(errmsg);
                cb_err(new fmacloud.Error(3202, errmsg));
                return;
            }

            var c_page = tpldata.get("pages");
            if (c_page.length <= 0 || c_page[0].get("item_object").length <= 0) {

                errmsg = "模版数据不正确（页数<" + c_page.length + ">或元素数<"
                        + c_page[0].get("item_object").length + ">异常）。";
            ...
            var SaveTplData = function () {
                //保存模版数据
                tpldata.save().then(function () {

                    if (tplobj.get("tpl_type") == fmaobj.TPL_CNT_APP) {

                        c_page = tpldata.get("pages");
                        var item = c_page[0].get("item_object")[0];

                        tplobj.set("icon_img",
                                fmacapi.tpl_res_img_url(
                                        tplobj.get("tpl_id"),
                                        item.get("item_cntype"),
                                        item.get("item_val")));
                    }

                    tplobj.save().then(function () {
                        cb_ok({ "tplobj": tplobj, "tpldata": tpldata, "group": group_object });
                    }, cb_err);
                }, cb_err);
            }

            var saveGroup = function (index) {
                if (index < c_groups.length) {
                    c_groups[index].save().then(function (gobj) {
                        group_object.push(gobj);
                        index++;
                        saveGroup(index);
                    }, cb_err);
                } else {
            ...
            var SaveObjecets = function () {
                //判断tpl_sign状态，0-默认，1-贺卡,2-ME刊,tpl_sign=2 为ME微刊,微刊的页数据必须包含页所属组对象id
                if (tplobj.get("tpl_sign")) {
                    if (tpldata.get("group") && tplobj.get("tpl_sign") >1) {
                        c_groups = tpldata.get("group");
                        saveGroup(0);
                    } else {
                        SaveTplData();
                    }
            ...
            /*
             // 保存效果图
             */
            fmacloud.tpl_save_effect_img(tplobj, tpl_imgs.effect_img, function () {

                if (tpl_imgs.elem_imgs == null || tpl_imgs.elem_imgs.length == 0) {
                ...
                //保存图片元素文件资源
                fmacloud.execute_array_task_async(tpl_imgs.elem_imgs, 0,
                        function (obj, cb_task_ok, cb_task_err) {

                            var filename;

                            var page = tpldata.get("pages")[obj[0]];
                            var elems = page.get("item_object");
                            var elem = elems[obj[1]];
                            if (elem.get("item_cntype") == 2) {

                                //链接地址
                                var val = elem.get("item_val");
                                var ipos = val.lastIndexOf('/');
                                filename = val.substr(ipos + 1, val.length - ipos - 1);
                                ...
                                filename = fmacapi_create_uid();
                            }

                            file = new fmacloud.File(filename + ".jpg", { base64: obj[2] });
                            file.save().then(function () {
    ...
    fmacloud.tpl_save_all_expand = function (tplobj, tpldata, tpl_imgs, cb_ok, cb_err,cb_progress) {
        ...
        try {
            ...
            var SaveObjecets = function(){
                ...
                tpldata.save().then(function () {
                    tplobj.save().then(function () {
                        cb_ok({ "tplobj": tplobj, "tpldata": tpldata });
                    }, cb_err);
                }, cb_err);
            };
            ...
            fmacloud.tpl_save_effect_img(tplobj, tpl_imgs.effect_img, function () {
                ...
                fmacloud.execute_array_task_async(tpl_imgs.elem_imgs, 0,
                    function (obj, cb_task_ok, cb_task_err) {
                       ...
                       var file = new fmacloud.File(filename + ".jpg", { base64: obj[2] });
                        file.save().then(function () {
                            elem.set("item_cntype", 2);
                            elem.set("item_val", file.url());
                            cb_task_ok(obj);
    ...
    fmacloud.tpl_save_all = function (tplobj, tpldata, tpl_imgs, cb_ok, cb_err, cb_progress) {
    ...
    /*
     //从服务器读取图片BASE64编码
     //回调：cb_ok(base64_string),cb_err(error)
     */
    fmacloud.request_img_base64 = function (remote_url, cb_ok, cb_err) {

        var fn = remote_url.substr(remote_url.indexOf('/filestore'));
        var apiurl = fms_api_url + "?mod=fms_img&act=tobase64&file=" + fn;
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                else cb_err(obj.error);

            },
            error: function (XMLHttpRequest, textStatus, errorThrown) {
                cb_err(textStatus || errorThrown.message);
            }
        });
    };
    ...
    fmacloud.copy_remote_resource = function (params, cb_ok, cb_err) {
        ...
        if (elem.get("item_cntype") == 2) {
            ...
            var val = elem.get("item_val");
            var ipos = val.lastIndexOf('/');
            origfilename = val.substr(ipos + 1);
        ...
        var barename = remote_url.substr(pos + 1);
        filename = key_id + "." + barename;
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (obj.status) {
                    ...
                    var file = new fmacloud.File(barename + ".jpg", { base64: obj.data });
    ...
    fmacloud.live_update = function (cb_ok, cb_err) {

        /*
         //server_version变量定义在./dl/update_version.js文件里
         */
        if (server_version) {
            // 当前版本号
            var current_version = "0.8.39";

            // 取得服务器端的版本号
            var get_server_version = function () {
                if (server_version != current_version) {
                    if (confirm("系统检测到新版本，是否现在进行更新？")) {
                        setTimeout(update_system, 10000);
                    }
                }
            }
            ...
            var update_system = function () {
                fmantv_download_package("http://me.gli.cn/dl/update.zip",
                    function () {
                        alert("更新成功，请重新进入系统！");
                    },
                    function () {
...
//
/////////////////////////////////////////////////////
    fmacloud.ConvertJson2Tpldata = function(jdata){
       try {
        var tpldata = fmaobj.tpl_data.create();

        for ( var attr in jdata ) {

            if ( attr == "pages" ){

                var pages = jdata[attr];
                var pagesdata = [];
                
                for(var i=0; i<pages.length; i++){ 
                    var page = pages[i];
                    var pagedata = fmaobj.page.create();

                    for ( var pttr in page ){
                        ...
                        if ( pttr == "item_object"){
                            ...
                            for (var j=0; j<items.length; j++){
                                ...
                                var item = items[j];
                                var itemdata = fmaobj.elem.create();
                                for ( var ettr in item ){
                                    itemdata.set(ettr,item[ettr]);
                                }

                                itemsdata[j] = itemdata;

                            }

                            pagedata.set("item_object",itemsdata);

                        }else{

                            pagedata.set(pttr,page[pttr]);
                        }
                ...
                tpldata.set("pages",pagesdata);

            }else{
                tpldata.set(attr,jdata[attr]);
            }
        }
    } catch (e) {
    ...
    fmacloud.ConvertTpldata2Json = function(tpldata){
    ...
    fmacloud.cld_get_tpl_data = function(tplid,format,cbok,cberr,incReadCount) {
        incReadCount = incReadCount || false;
        fmacloud.Cloud.run('cld_get_tpl_data', {"tplid":tplid,"incRead":incReadCount}, {
            success: function (tpldata) {

                var tdata = (format=="AVOS")?fmacloud.ConvertJson2Tpldata(tpldata):tpldata;
                cbok(tdata);
    ...
    fmacloud.search_music = function(keyword,cbok,cberr) {
        keyword = keyword || "爱";
        fmacloud.Cloud.run('search_music', {"tn":keyword}, {
    ...
    fmacloud.confirm_music_link = function(linkid,cbok,cberr) {
        linkid = linkid || "invalid parameters";
        fmacloud.Cloud.run('confirm_music_link', {"urlid":linkid}, {
...
/////////////////////////////////////////////////////
    ...
    /**
     *保存动效卡对象
     * 参数，deobj，动效卡对象(fmaobj.decard对象)
     *      pic1_base64, 图片1的base64字符串，可以为null，表示图片1的数据无须更新
     *      pic2_base64, 图片2的base64字符串，可以为null，表示图片2的数据无须更新
     *      pic3_base64, 图片3的base64字符串，可以为null，表示图片3的数据无须更新
     *      cb_ok(deobj) 成功后，异步回调接口返回原decard obj
     *      cb_err(err) 调用失败后，异步回调接口返回,err包含错误信息（字符串）
     * 说明：保存成功之后，decard obj里的pic1,pic2,pic3属性值将改变为图片URL
     */
    fmacloud.save_decard = function(deobj,pic1_base64,pic2_base64,pic3_base64,cb_ok,cb_err){

        var err_callback = function(err){cb_err(err.message)};

        if ( pic1_base64 && pic1_base64 != "") {
            //保存图片1
            fmacloud.save_image(".jpg", pic1_base64, function (file1) {

                deobj.set("pic1",file1);
                if ( pic2_base64 && pic2_base64 != "") {
                    //保存图片1
                    fmacloud.save_image(".jpg", pic2_base64, function (file2) {

                        deobj.set("pic2",file2);
                        if ( pic3_base64 && pic3_base64 != "") {

                            fmacloud.save_image(".jpg", pic3_base64, function (file3) {

                                deobj.set("pic3",file3);
                                deobj.save().then(cb_ok,err_callback);
                            },cb_err);
    ...
    /**
     *读取动效卡对象
     * 参数，decard_ObjectId，动效卡对象的Object ID
     *      cb_ok(deobj) 成功后，异步回调接口返回获取到的decard obj
     *      cb_err(err) 调用失败后，异步回调接口返回,err包含错误信息（字符串）
     */
    fmacloud.read_decard = function(decard_ObjectId,cb_ok,cb_err){

        var decard = fmacloud.Object.extend("decard");
        var query = new fmacloud.Query(decard);
        return query.get(decard_ObjectId).then(cb_ok,function(err) {
            cb_err(err.message);
        });
    };
...
/////////////////////////////////////////////////////
    ...
    /**
     *读取H5微场景对象
     * 参数，mag_ObjectId，H5微场景对象的Object ID
     *      cb_ok(magobj) 成功后，异步回调接口返回获取到的mobimag obj
     *      cb_err(err) 调用失败后，异步回调接口返回,err包含错误信息（字符串）
     */
    fmacloud.read_mobimag = function(mag_ObjectId,cb_ok,cb_err){

        var mobimag = fmacloud.Object.extend("mobimag");
        var query = new fmacloud.Query(mobimag);
        return query.get(mag_ObjectId).then(function(magobj){

            var tpls = magobj.get("tplobjs");
            fmacloud._fetch_object(tpls, 0, cb_ok, function(err){
                cb_err(err.message);
            });
...
/////////////////////////////////////////////////////
    ...
    fmaobj.page = {
        ...
        create: function () {
            ...
            page.set("line_width", 0);
            page.set("line_color", '0');
            page.set("line_radius", 'none');
            page.set("item_filter", 'none');
            page.set("item_filter_val", 'none');
            page.set("item_int", 0);
    ...
    fmaobj.tpl = {
        ...
        create: function () {
            ...
            tplobj.set("mobimag_id","");    //所属H5微场景的ID
    ...
    fmaobj.labelobj = {
        ...
        create: function () {
            ...
            var label = new (fmacloud.Object.extend("labels"));

            label.set("name", 'none');
            label.set("type", "");
            label.set("order", 0);
            label.set("color", "#ffffff");
            label.set("img", "");
    ...
    fmaobj.decard = {
        ...
        create: function () {
            ...
            var card = new (fmacloud.Object.extend("decard"));

            card.set("detpl_no",0); //动效卡模版编号
            card.set("detpl_id","");//动效卡模版ID
            card.set("card_id",fmacapi_create_uid());    //动效卡ID
            card.set("peername", "");   //对方姓名
            card.set("myname", "");     //发送方姓名
            card.set("msg1", "");       //消息内容1
            card.set("msg2", "");       //消息内容2
            card.set("msg3", "");       //消息内容3
            card.set("pic1", null);       //图片1
            card.set("pic2", null);       //图片2
            card.set("pic3", null);       //图片3
    ...
    fmaobj.Appconf = {
        ...
        create: function () {
            ...
            var conf = new (fmacloud.Object.extend("appconf"));

            conf.set("home_res",""); //首页背景资源（可以是图片、动画、视频)
            conf.set("home_res2","");//起始页第2页背景资源（可以是图片、动画、视频)
            conf.set("home_music","");//起始页前景音乐对象id
            conf.set("font_server","");//字体服务器URL
            conf.set("res_server",""); //资源服务器URL
    ...
    fmaobj.pvrecord = {
        ...
        create: function () {
            ...
            var pv = new (fmacloud.Object.extend("pvrecord"));

            pv.set("pvtype",0); //type,0:模版/1作品/11动效卡
            pv.set("objid",""); //模版/作品/动效卡的ID
            pv.set("user_id","none");   //用户ID，如为"none"则是游客
            pv.set("user_ip","");       //用户IP地址
            pv.set("pv_time","");       //用户浏览的日期时间
            pv.set("user_make",false);  //用户是否制作
            pv.set("user_save",false);  //用户是否保存
            pv.set("user_share",false); //用户是否分享
...
function fmacapi_data_hit(channel,para1,para2,cb_ok){
    ...
    fmacloud.initialize(channel,para1,para2);

    fmacapi_init_fmaobj();
    fmacapi_init_interface();

    var thisEnv = (channel == web_channel) ?  fmacapi.env_web : fmacapi.env_app;
    var isTesting = (channel == test_channel );

    fmacapi.initialize(thisEnv,isTesting);
    cb_ok();
}

function fmacapi_loadlogo(cb_ok,cb_err){

    cb_ok = cb_ok || function(){};
    cb_err = cb_err || function(){};

    var fmawr_local = typeof(fmawr) === "undefined" ? "0" : fmawr;
    var testChannel = fmawr_local;
    var p1,p2;
    ...
    switch(fmawr_local){
        ...
        //v1.2正式服for me悦读
        case "997":
            p1 = "b4acac58be2ad39fa70e1681369d50cd39a6ad595555e57e0041373e";
            p2 = "e981e8ab73b7fce3fb8ddbef31e31a41fcf561910ae9a17e006b161d";
            break;
        //v1.1
        case "998":
            p1 = "df66a8b139f70bce069cdae71d781ca2923a4e84ad9fbbcbcd5d621ed0c4d5470dbd85be5b34a87d677169cfefa8de2e011e62e8";
            p2 = "12babf6ad3d5eca0e9b6c219b79e2fddf7119c3f5ad68925d1e52c7ada3076270e5dfdf44c6bbaf2086a2d1caeb3aa4a013c7998";
            break;
        //v1.2正式服
        case "999":
            p1 = "8ab78cbc129fb17b520d5cd77e61173c71cab262130d7cdc86044b9bddb6b36af4666b519e21311084e1c8de13aa27640453be25";
            p2 = "821b8166246b60387438748739b11271a37f68488aac0c33cf1dff11bb324c6d2411d0a4c845be7c9d17a1e259dee4520552d734";
            break;
        case "10000":
            p1 = "589a4b79dfcb4d39687fada2ccebf83c6d1da27844de0c0a10004d3bb3aa84ae060151dac7";
            p2 = "5c52cc6a45f7a55cf5c2a8f79f006f8c3ff7bc679b04193b00993848";
            break;
        //v1.2测试服
        case "0":
        default:
            p1 = "e5a2bfda22e839369ca5dd499bda6c423adca418bf415cf65b802fd515f6cce7b0f28546579a72570c2b6dab4b6c7b0803db9a2f";
            p2 = "d6b6dfea1247d16e367a48f15a6a33f967dd3e6c8d8aa53184b94b01ccafb615d973d6e47ce48dbc4bfe910e3207878d01b140cd";
            break;
    }
    ...
    if ( fmacapi_is_native() ) {
        ...
        setTimeout(function(){
            ...
            if ( !callbacked ){
                cb_err("初始化程序异常中断(于位置"+calbackstep+")。");
            }
        ...
        fmantv_init("js/common/api/", function () {
            ...
            calbackstep = 1;
            fmantv_get_build_info(function (channel, para1, para2) {

                calbackstep = 2;
                callbacked = true;

                fmacapi_data_hit(channel, para1, para2, cb_ok);
//                fmacapi_data_hit(
//                    channel,
//                    fmacapi_get_data(p1, testChannel),
//                    fmacapi_get_data(p2, testChannel),
//                    cb_ok
//                );
                ...
                callbacked = true;
                fmacapi_data_hit(
                    parseInt(testChannel),
                    fmacapi_get_data(p1, testChannel),
                    fmacapi_get_data(p2, testChannel),
                    cb_ok
                );

                fmacapi.set_last_error("获取渠道号失败，原因是：" + err);
            });
            ...
            fmacapi.set_last_error("本机模块初始化失败!原因是：" + err);
        });
    }else{
...
/////////////////////////////////////////////////////
        ...
        initialize: function(thisEnv,isTesting){
            this.cur_env = thisEnv;
...
/////////////////////////////////////////////////////
         ...
         *fmaobj.TPL_MULTI_CHECKED = 13;  //已通过审核的多页作品
         *fmaobj.TPL_CNT_APP  = 20;       //内容应用
         *数组示例: fma_type_tpl_and_works = [0,1] //单页模版及作品
         *selector:查询选择器，其取值为：
         *          'editor rec'-编辑推荐（按推荐号排序）
         *          'author rec'-作者推荐（按推荐号排序）
         *          'label:xxx' -根据标签选择(多个标签用冒号隔开）
         ...
         *@cb_err(err) 调用失败后，异步回调接口返回,err包含错误信息（字符串）
         */
        labels_query: function (key,value,orderby,isAscending,skip,limit,cb_ok,cb_err) {

            fmacloud.query_labels(key,value,orderby,isAscending,skip,limit,cb_ok,cb_err);
        },
        ...
        tpl_get_tpl: function (tpl_works_id,cb_ok,cb_err) {
            ...
            //获取云端数据
            fmacloud.get_tpl_works_obj(tpl_works_id,cb_ok,cb_err,false);
        },

        /**
         *获得模版/作品/签章的模版对象(Json格式)
         *tpl_works_id:模版对象或作品对象id：
         *@cb_ok(data_obj) 成功后，异步回调接口返回tplobj模版/作品对象(Json格式)
         *@cb_err(err) 调用失败后，异步回调接口返回,err包含错误信息（字符串）
         */
        tpl_get_tpl_json: function (tpl_works_id, cb_ok,cb_err,incReadCount) {

            //获取云端数据
            fmacloud.get_tpl_works_obj(tpl_works_id,function(tplobj){
                cb_ok(tplobj.toJSON());
            },cb_err,incReadCount);
        },
        ...
        tpl_get_data: function (tpl_works_id,cb_ok,cb_err) {
            ...
            //获取云端数据
             fmacloud.cld_get_tpl_data(tpl_works_id,"AVOS",cb_ok,cb_err,false);
        },

        /**
         *获得模版/作品/签章的页数据(返回Json对象)
         *tpl_works_id:模版对象或作品对象id：
         *@cb_ok(data_obj) 成功后，异步回调接口返回data_obj模版/作品页数据对象(Json格式)
         *@cb_err(err) 调用失败后，异步回调接口返回,err包含错误信息（字符串）
         */
        tpl_get_data_json: function (tpl_works_id,cb_ok,cb_err) {

            //获取云端数据
            fmacloud.cld_get_tpl_data(tpl_works_id,"Json",cb_ok,cb_err,false);
        },
        ...
        /**
         *获得模版/作品/签章带水印的效果图
         * img_url: 要加水印的图片url，目前只支持存储在云端的图片
         * dx: 水印的横向边距；dy: 水印的纵向边距
         * opacity: 透明度，0-100,100为全不透明，默认为100
         * position: 水印放置的位置，可选值有：NorthWest, North, NorthEast,
         *              West, Center, East, SouthWest, South, SouthEast
         *              默认为"SouthEast"，即右下角
         * cb_ok: 成功的回调函数，形如cb_ok(url)
         * cb_err: 失败的回调函数，形如cb_err(error)
         */
        tpl_withlogo_img_url:function(img_url,logo_url,dx,dy,cb_ok,cb_err,opacity,position){

            return fmacloud.tpl_withlogo_img_url(img_url,logo_url,dx,dy,cb_ok,cb_err,opacity,position);
         ...
         *获得模版/作品/签章分享图标
         */
        tpl_share_img_url:function(tplobj){

            var url = tplobj.get("tpl_share_img");
            if ( !url ) return this.tpl_thumb_img_url(tplobj);

            if ( url.substr(0,3)=="AV:"){
                return url.slice(3);
            }else {
                //return fms_res_path + fms_tpl_store
                //    + tplobj.get("tpl_id") + "/" + tplobj.get("tpl_share_img");
                return url;
            }
        },
         ...
         *获得模版/作品/应用的icon图标
         */
        tpl_icon_img_url:function(tplobj){

            var url = tplobj.get("icon_img");
            if ( !url ) return this.tpl_share_img_url(tplobj);
         ...
         *保存效果图及其缩略图
         *  tplobj:模版/作品签章对象(类型为fmaobj.tpl)：
         *  img_base64_str,效果图的base64数据，若img_base64_str为null或""，则会直接调用cb_ok(tplobj)
         **/
        tpl_save_effect_img : function(tplobj,img_base64_str,cb_ok,cb_err){

            return fmacloud.tpl_save_effect_img(tplobj,img_base64_str,cb_ok,cb_err);
        },
       ...
       tpl_save_all: function (tplobj,tpldata,tpl_imgs,cb_ok,cb_err) {
        ...
        //
        /////////////////////////////////////////////
       tpl_save_all_expand: function (tplobj, tpldata, tpl_imgs, cb_ok, cb_err) {

           //本机保存
           //云端保存
           return fmacloud.tpl_save_all_expand(tplobj, tpldata, tpl_imgs, cb_ok, cb_err);
       },
        /**
         *保存模版/作品/签章对象
         *tplobj:模版/作品签章对象(类型为fmaobj.tpl)：
         ...
         *      从5-8分别是：
         *      ["A-5","主题5区"],["A-6","主题6区"],["A-7","主题7区"],["A-8","主题8区"]];
...
/////////////////////////////////////////////////////
        ...
        /**
        //增加模版/作品/应用的分享计数
        //参数：tpl_id，模版/作品/应用的ID
        //     cb_ok(tplobj) 成功后，异步回调接口返回tplobj对象,本参数可以为null
        //     cb_err(err) 调用失败后，异步回调接口返回,err包含错误信息（字符串），本参数可以为null
        */
        inc_share_int: function(tpl_id,cb_ok,cb_err){

            cb_ok = cb_ok || {};
            cb_err = cb_err || {};

            this.tpl_get_tpl(tpl_id,function(tplobj){

                tplobj.set(tplobj.get("share_int")+1);
                tplobj.save().then(function(){
                    cb_ok(tplobj);
                },function(err){
                    cb_err(err.message);
        ...
        /**
         //更新tplobj对象的部分属性
         //参数：tpl_id，模版/作品/应用的ID
         //     props(js对象，可包含tplobj的部分需更新属性，调用时需注意属性的名称与类型的正确性)
         //     cb_ok(tplobj) 成功后，异步回调接口返回tplobj对象,本参数可以为null
         //     cb_err(err) 调用失败后，异步回调接口返回,err包含错误信息（字符串），本参数可以为null
         */
        update_tplobj: function(tpl_id,props,cb_ok,cb_err){
...
/////////////////////////////////////////////////////
        ...
        /**
         //将一个Json格式的tpldata对象转换为TplData对象
         //参数：jdata,Json格式的tpldata对象
         //     说明：返回的tpldata对象可能需要另行销毁，不然可能形成内存泄露
         */
        convert_josn_2_tpldata : function(jdata){
            return fmacloud.ConvertJson2Tpldata(jdata);
        },

        /**
         //将一个tpldata对象转换为Json格式对象
         //参数：tpldata, tpldata对象
         //     返回：Json格式的tpldata对象
         */
        convert_tpldata_2_json : function(tpldata){
            return fmacloud.ConvertTpldata2Json(tpldata);
        },
...
/////////////////////////////////////////////////////
         ...
         *保存动效卡对象
         * 参数，deobj，动效卡对象(fmaobj.decard对象)
         *      pic1_base64, 图片1的base64字符串，可以为null，表示没有图片1
         *      pic2_base64, 图片2的base64字符串，可以为null，表示没有图片2
         *      pic3_base64, 图片3的base64字符串，可以为null，表示没有图片3
         *      cb_ok(deobj) 成功后，异步回调接口返回原decard obj
         *      cb_err(err) 调用失败后，异步回调接口返回,err包含错误信息（字符串）
         * 说明：保存成功之后，decard obj里的pic1,pic2,pic3属性值将改变为图片URL
         */
        decard_save : function(deobj,pic1_base64,pic2_base64,pic3_base64,cb_ok,cb_err){

            return fmacloud.save_decard(deobj,pic1_base64,pic2_base64,pic3_base64,cb_ok,cb_err);
        },
         ...
         *      cb_err(err) 调用失败后，异步回调接口返回,err包含错误信息（字符串）
         */
        decard_read : function(decard_ObjectId,cb_ok,cb_err){
            return fmacloud.read_decard(decard_ObjectId,cb_ok,cb_err);
        },
...
/////////////////////////////////////////////////////
        ...
        /**
         *加载云字体(此方法需要jQuery)
         * 参数，fontserver : 云字体服务器地址，形如：http://agoodme.com:3000
         *      type : 云字体类型，有两种："fixed"-永久固化的云字体；"temp"-临时使用的云字体
         *      font : 字体名称，例如"中山行书",详见《支持的云字体列表》
         *      text : 待使用云字体的文本，最多不超过256个汉字
         *      cb_ok : 成功回调函数，其返回fontobj对象,包含以下属性：
         *                status: 状态码,0为成功，其它值为失败
         *               [成功]: font: 云字体名称，用于font-family
         *                       src: 云字体url
         *                       type: 字体类型，例如"truetype"
         *               [失败]: msg: 错误信息
         *      cb_err : 失败回调函数，返回错误信息
         */
        cfs_loadfont: function(fontserver, type, font, text, cb_ok,cb_err){

            $.ajax({
                type: 'GET',
                url: fontserver + "/loadfont/?callback=?&r=" + Math.random(),
                data: {"type":type,"font":font,"text":text},
                success: cb_ok,
        ...
        /**
         *加载云字体风格
         * 参数，font : 云字体风格名称，可使用云字体服务器返回的字体名，也可自定义，自定义时需要避免重复
         *      src : 云字体URL
         *      type : 字体类型，由云字体服务器返回的字体类型，例如"truetype"
         *      id : 字体风格节点的id，自定义，例如"txt_id1"，需要保持唯一
         */
        cfs_load_font_style:function(font,src,type,id){
            var iHead = document.getElementsByTagName('HEAD').item(0);
            var iStyle= document.createElement("style");
            iStyle.innerHTML = "@font-face {font-family:"+font+"; src: url('"+src+"') format('"+type+"');}";
            if ( id && id != ""){
                iStyle.setAttribute("id",id);
            }
...
/////////////////////////////////////////////////////
        ...
        /**
         *搜索在线音乐
         * 参数，keyword : 可以是音乐名、歌手名、专辑名，支持模糊搜索
         *
         * 返回：Json对象数组，每个对象包含以下属性：
         *      title :  音乐名
         *      singer :  歌手名
         *      author :  作者名
         *      album  :  专辑
         *      from :    音乐来源
         *      link :    音乐的linkid,需要用此id调用ms_confirm_music_link获取真正的音乐链接
         *
         */
        ms_search_music:function(keyword,cb_ok,cb_err){

            fmacloud.search_music(keyword,cb_ok,cb_err);
        },

        /**
         *确认并获取有效的音乐链接
         * 参数，linkid : 音乐的链接id，由ms_search_music返回
         *
         * 返回：由cb_ok回调返回有效的音乐链接
         */
        ms_confirm_music_link:function(linkid,cb_ok,cb_err){

            fmacloud.confirm_music_link(linkid,cb_ok,cb_err);
        },
...
/////////////////////////////////////////////////////
        /**
         *读取ME App系统配置对象
         * 参数，cb_ok(conf) 成功后，异步回调接口返回获取到的Appconf obj
             conf.get("home_res"); //首页背景资源（可以是图片、动画、视频)
             conf.get("home_res2");//起始页第2页背景资源（可以是图片、动画、视频)
             conf.get("home_music");//起始页前景音乐对象id
             conf.get("font_server");//字体服务器URL
             conf.get("res_server"); //资源服务器URL
         *
         *  cb_err(err) 调用失败后，异步回调接口返回,err包含错误信息（字符串）
         */
        read_Appconf : function(cb_ok,cb_err){

            //查询云端模版/作品
            var conf = fmacloud.Object.extend("appconf");
            var query = new fmacloud.Query(conf);

            query.find({success: function(confobjs){
...
/////////////////////////////////////////////////////
        ...
        //获取存在LocalStorage的最后一个错误信息
        get_last_error: function(){
            return window.localStorage.fmacapi_last_error || "OK";
        },

        //在LocalStorage存储最后一个错误信息
        set_last_error: function(err_text){
            window.localStorage.fmacapi_last_error = err_text;
        },
...
/////////////////////////////////////////////////////
    ...
    ["A-5","主题5区"],["A-6","主题6区"],["A-7","主题7区"],["A-8","主题8区"],["A-9","主题9区"],["A-10","主题10区"],
        ["B-1","广场瀑布流"],["B-2","广场焦点图"],["B-3","主题焦点图"],["B-4","保留区"],
        ["B-5","保留区"],["B-6","保留区"],["B-7","保留区"],["B-8","保留区"],["B-9","保留区"],["B-10","保留区"],
        ["C-1","首页应用区"],["C-2","主题应用区"],["C-3","主题一级内容(热点)"],["C-4","主题二级内容(专栏)"],
        ["C-5","主题一级内容(精选)"],["C-6","保留区"],["C-7","保留区"],["C-8","保留区"],["C-9","保留区"],["C-10","保留区"]
    ];
...
(function(){
    ...
    fmacapi_loadlogo(function () {
       ...
       // if ( me_logo_ok ) {
        if ("undefined" != typeof me_logo_ok) { 
            var iHead = document.getElementsByTagName('HEAD').item(0);
            var iScript = document.createElement("script");
            iScript.type = "text/javascript";
            iScript.setAttribute("data-main", "js/main");
            iScript.src = "js/lib/require/require.js";
            iHead.appendChild(iScript);
        }
